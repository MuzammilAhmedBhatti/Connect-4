...
// public:
//     void do_a_move(int**& grid, int col_num, int mark, int rows, int cols) {
...
//         return copied_board;
//     }
//     bool checkIfWinningMove(int**& grid, int mark, int rows, int cols) {
//         int winPattern = 0;
//         // for horizontal checks
//         for (int column = 0; column < cols - 3; column++) {
//             for (int row = 0; row < rows; row++) {
//                 for (int i = 0; i < 4; i++) {
//                     if (grid[row][column + i] == mark) {
//                         winPattern++;
//                     }
//                     if (winPattern == 4) {
//                         return true;
...
//         return false;
//     }
//     int scoreGivingFunction(int good, int bad, int empty) {
//         int score = 0;
//         if (good == 4) { score += 500001; }
//         else if (good == 3 && empty == 1) { score += 5000 + should_win; }
//         else if (good == 2 && empty == 2) { score += 500 + should_win; }
//         else if (bad == 2 && empty == 2) { score -= 500 + !should_win; }
//         else if (bad == 3 && empty == 1) { score -= 5000 + !should_win; }
//         else if (bad == 4) { score -= 500000; }
//         return score;
//     }
//     int givingScoreToSet(int* set, int mark) {
//         int good = 0;
//         int bad = 0;
//         int empty = 0;
//         for (int i = 0; i < 4; i++) {
...
//         }
//         bad -= good;
//         return scoreGivingFunction(good, bad, empty);
//     }
//     int scoreOfSet(int** grid, int mark, int rows, int cols) {
...
//             for (int column = 0; column < cols - 3; column++) {
//                 for (int i = 0; i < 4; i++) {
//                     set[i] = row_arr[column + i];
//                 }
//                 score += givingScoreToSet(set, mark);
//             }
//         }
//         // vertical
//         for (int column = 0; column < cols; column++) {
...
//             result[0] = scoreOfSet(grid, 2, rows, cols);
//             result[1] = -1;
//             return result;
//         }
...
//             if (checkIfWinningMove(grid, 1, rows, cols)) {
//                 return movesDoneTillNow;
//             }
//             for (int column = 0; column < cols; column++) {
//                 if (grid[0][column] == 0) {
//                     int** updatedBoard = copyBoard(grid, rows, cols);
//                     do_a_move(updatedBoard, column, mark, rows, cols);
//                     int* score = miniMax(updatedBoard, depth - 1, alf, bet, 1, rows, cols);
//                     for (int i = 0; i < rows; i++) {
//                         delete[] updatedBoard[i];
//                     }
//                     delete[] updatedBoard;
//                     if (score[0] > movesDoneTillNow[0]) {
...
//             movesDoneTillNow[1] = -1;
//             if (checkIfWinningMove(grid, 2, rows, cols)) {
//                 return movesDoneTillNow;
//             }
//             for (int column = 0; column < cols; column++) {
//                 if (grid[0][column] == 0) {
...
//     {
//         float texture_width = GetScreenWidth() / static_cast<float>(COLS);
//         float texture_height = GetScreenHeight() / static_cast<float>(ROWS);
...
//         float texture_width = GetScreenWidth() / static_cast<float>(getCols());
//         float texture_height = GetScreenHeight() / static_cast<float>(getRows());
//         float texture_size = min(texture_width, texture_height);
